...
```rust
use muffin::os::{get_current_os, OsTier};
...
fn construct_path(parts: &[&str]) -> PathBuf {
    ...
    // On Windows: C:\Users\Name\muffin
    // On Unix:    /home/name/muffin
...
fn configure_features() {
    ...
    if os.tier() >= muffin::os::OsTier::Compatible {
...
```rust
use muffin::os::{get_current_os, PureRustFallback};

fn get_safe_adapter() -> Box<dyn muffin::os::OsAdapter> {
...
fn use_fallback_mode() {
    ...
    assert_eq!(fallback.tier(), muffin::os::OsTier::Legacy); // Conservative
...
fn process_with_optimization() {
    ...
    if os.tier() >= muffin::os::OsTier::Modern {
        // Use advanced features
        process_parallel()
    } else if os.tier() >= muffin::os::OsTier::Compatible {
...
fn get_feature_flags() -> HashMap<&'static str, bool> {
    ...
    flags.insert("unicode", os.tier() >= muffin::os::OsTier::Compatible);
    flags.insert("long_paths", os.tier() >= muffin::os::OsTier::Modern);
...
fn create_config() -> BuildConfig {
    ...
    BuildConfig {
        ...
        use_cache: os.tier() >= muffin::os::OsTier::Modern,
