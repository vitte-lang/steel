...
// /Users/vincent/Documents/Github/muffin/src/builder.rs
//! builder — orchestration layer between Muffin (config) and Steel (build rules) (std-only)
//!
//! Role in the pipeline
//! - `build muffin` resolves workspace configuration and emits a `.mcfg` (Muffinconfig)
//! - `build steel` consumes `.mcfg` + Steel rules to perform the actual build
//!
//! This module sits at the boundary:
//! - validates + normalizes resolved config
//! - packages resolved targets into a deterministic `.mcfg` text
//! - builds a deterministic dependency order for targets (toposort)
//! - provides a stable in-memory `BuildPlan` representation Steel (or tests) can consume
//!
//! Constraints
//! - std-only
...
//!   The execution step is delegated to Steel (or to a caller-provided backend).
...
///
/// This is intentionally narrow: it is what the builder needs to emit `.mcfg` and build a plan.
#[derive(Debug, Clone)]
pub struct SessionConfig {
    pub resolved: crate::build_muf::ResolvedConfig,
    pub targets: Vec<TargetDef>,

    /// `.mcfg` output path (if emission is requested).
...
pub struct BuildOutput {
    ...
    /// Deterministic `.mcfg` content (when stage >= EmitConfig).
...
///
/// Steel can consume this plan directly, or ignore it and just read `.mcfg`.
...
/// Build `.mcfg` text deterministically.
///
...
pub fn build_mcfg_text(cfg: &crate::build_muf::ResolvedConfig, targets: &[TargetDef]) -> String {
    ...
    out.push_str("# Muffinconfig (.mcfg)\n");
    out.push_str("# generated by build muffin\n\n");

    out.push_str(&format!("schema {}\n", cfg.schema_version));
    out.push_str(&format!("profile \"{}\"\n", escape(&cfg.profile)));
    out.push_str(&format!("target \"{}\"\n", escape(&cfg.target)));
    out.push_str(&format!("root \"{}\"\n", escape(&cfg.project_root.to_string_lossy())));
    out.push_str(&format!("muffinfile \"{}\"\n", escape(&cfg.muffinfile_path.to_string_lossy())));
