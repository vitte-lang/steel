...
// /Users/vincent/Documents/Github/flan/src/builder.rs
//! builder — orchestration layer between Flan (config) and the execution layer (std-only)
//!
//! Role in the pipeline
//! - `build flan` resolves workspace configuration and emits a `.mff` (Flanconfig)
...
/// It is not the FlanConfig language.
pub fn build_mcfg_text(cfg: &crate::build_muf::ResolvedConfig, targets: &[TargetDef]) -> String {
    let mut out = String::new();

    out.push_str("# Flanconfig (.mff)\n");
    out.push_str("# generated by build flan\n\n");

    out.push_str(&format!("schema {}\n", cfg.schema_version));
    out.push_str(&format!("profile \"{}\"\n", escape(&cfg.profile)));
    out.push_str(&format!("target \"{}\"\n", escape(&cfg.target)));
    out.push_str(&format!("root \"{}\"\n", escape(&cfg.project_root.to_string_lossy())));
    out.push_str(&format!("flanfile \"{}\"\n", escape(&cfg.flanfile_path.to_string_lossy())));
...
mod tests {
    ...
    fn emits_mcfg_contains_targets() {
        let mut cfg = build_muf::generate_default_mcfg("/tmp/flan");
    ...
    fn planning_toposort_respects_deps() {
        ...
        let ts = std::time::SystemTime::now()
            ...
            .as_nanos();
        root.push(format!("flan_builder_plan_{pid}_{ts}"));
        std::fs::create_dir_all(&root).unwrap();
        std::fs::write(root.join("FlanConfig"), "workspace ...\n").unwrap();
