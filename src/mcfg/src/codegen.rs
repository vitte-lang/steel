//! Muffin code generation.
//!
//! Responsibilities (convention):
//! - Emit the resolved configuration artifact: `Muffinconfig.mff` (stable, deterministic).
...
pub struct EmitOptions {
    pub kind: EmitKind,
    /// If `None`, caller decides default path (`./Muffinconfig.mff` for Mff).
    ...
    let (path, content) = match opts.kind {
        EmitKind::Mff => {
            let p = opts
                .out_path
                .clone()
                .unwrap_or_else(|| PathBuf::from("Muffinconfig.mff"));
